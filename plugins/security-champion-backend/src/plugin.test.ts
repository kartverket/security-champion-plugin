...
import { securityChampionBackendPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [securityChampionBackendPlugin],
    });

    await request(server).get('/api/security-champion-backend/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/security-champion-backend/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/security-champion-backend/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/security-champion-backend/todos/${createdTodoItem.id}`)
